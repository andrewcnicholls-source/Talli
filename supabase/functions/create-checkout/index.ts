...
//
//  A booking must return to the site it was made on. This used to default
//  to https://talli.co.nz when SITE_URL was unset, which meant the test
//  site's checkout sent paying customers to production, where the booking
//  does not exist and the confirmation page shows an error. A default that
//  is silently wrong for one environment is worse than no default: nothing
//  looks broken until someone has already paid.
//
//  So: SITE_URL is required, and RETURN_ORIGINS optionally widens it for
//  branch deploys. The request's Origin is honoured only when it appears on
//  that list — never on its own say-so, because this value lands in a Stripe
//  session and an unchecked redirect there is a phishing gift.
// ---------------------------------------------------------------------
const trimSlash = (s: string) => s.trim().replace(/\/+$/, '')

const SITE_URL = trimSlash(Deno.env.get('SITE_URL') ?? '')

const RETURN_ORIGINS: string[] = [
  SITE_URL,
  ...(Deno.env.get('RETURN_ORIGINS') ?? '').split(',').map(trimSlash),
].filter(Boolean)

// SITE_URL is the canonical address, used whenever the caller's Origin is
// absent or unrecognised. Null means "not configured" — never a guess.
function returnOrigin(req: Request): string | null {
  if (!SITE_URL) return null
  const origin = trimSlash(req.headers.get('Origin') ?? '')
  return origin && RETURN_ORIGINS.includes(origin) ? origin : SITE_URL
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error(err)
    return json({
      error: 'Payments are not configured on this project yet',
      code: 'NOT_CONFIGURED',
    }, 503)
  }

  // Checked before a bay is held, so a misconfigured project turns nobody's
  // booking into an orphaned hold.
  const siteUrl = returnOrigin(req)
  if (!siteUrl) {
    console.error('SITE_URL is not set on this project — refusing to guess a return address')
    return json({
      error: 'Payments are not configured on this project yet',
      code: 'NOT_CONFIGURED',
    }, 503)
